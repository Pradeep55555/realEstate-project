      ...
      <table className="w-full table-auto border-separate border-spacing-y-[2px] text-sm">
        ...
        <tbody>
          {currentSellers.map((seller, index) => (
            <tr key={seller.mobile} className="text-start bg-white shadow-sm rounded">
              <td className="py-2 px-4 whitespace-nowrap">{indexOfFirst + index + 1}</td>
              <td className="py-2 px-4 whitespace-nowrap">{seller.full_name}</td>
              <td className="py-2 px-4 whitespace-nowrap">{seller.mobile}</td>
              <td className="py-2 px-4 whitespace-nowrap">
                <label className="inline-flex items-center cursor-pointer">
                  <input
                    ...
                    onChange={() => handleToggleStatus(seller.mobile, seller.status)}
                  />
              ...
              <td className="py-2 px-4 whitespace-nowrap">{dayjs(seller.created_on).format('DD MMM YYYY')}</td>
              <td className="py-2 px-4 whitespace-nowrap space-x-2 flex justify-center">
                <button title="View" onClick={() => openModal(seller, false)} className="text-blue-500 hover:text-blue-700 cursor-pointer">
                  <Eye size={18} />
                </button>
                <button title="Edit" onClick={() => openModal(seller, true)} className="text-yellow-500 hover:text-yellow-700 cursor-pointer">
                  <Pencil size={18} />
                </button>
                <button title="Delete" onClick={() => handleDelete(seller.mobile)} className="text-red-500 hover:text-red-700 cursor-pointer">
                  <Trash2 size={18} />
                </button>
              </td>
            </tr>
